...
int tareCells(int repeats) {
  scale.set_gain(128);
  long reading = scale.read_average(repeats);
  offset_A = reading;
  scale.set_gain(32);
...
int calibrationRead() {
  ...
  if (weight1 && weight2) {
    calibration(50, weight1, weight2);
...
int calibration(int repeats, int weight1, int weight2) {
  Serial.println("Place weight 1 centered on scale and send command 1.");
  while (!Serial.available()); //wait for command to be sent
  scale.set_gain(128);
  long readingA1 = scale.read_average(repeats);
  Serial.println(readingA1 - offset_A);
  scale.set_gain(32);
  long readingB1 = scale.read_average(repeats);
  Serial.println(readingB1 - offset_B);

  Serial.println("Place weight 2 centered on scale and send command 2.");
  while (!Serial.available()); //wait for command to be sent
  scale.set_gain(128);
  long readingA2 = scale.read_average(repeats);
  Serial.println(readingA2 - offset_A);
  scale.set_gain(32);
  long readingB2 = scale.read_average(repeats);
  Serial.println(readingB2 - offset_B);
