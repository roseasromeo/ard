...
void readRegulator(int repeats) {
  // Read in voltage from the regulator and convert to pressure
  int reading_sum = 0;
  int regulatorpin;
  
  if (full_setup) {
    if (path) {
      regulatorpin = regulatorA_out_pin;
    }
    else {
      regulatorpin = regulatorB_out_pin;
    }
  }
  else {
    regulatorpin = regulatorA_out_pin;
  ...
  for (int i = 0; i < repeats; i++) {
    reading_sum = reading_sum + analogRead(regulatorpin);
  }
  float reading = (float)reading_sum/(float)repeats;
  Serial.print(((reading * 5.0/1024.0) - 1.0) * (p_max-p_min)/4.0, 4);
}
